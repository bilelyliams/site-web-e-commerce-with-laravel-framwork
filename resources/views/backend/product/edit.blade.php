      ...
      <form method="post" action="{{route('product.update',$product->id)}}">
        ...
        <div class="form-group">
          ...
          <select name="cat_id" id="cat_id" class="form-control">
              ...
              @foreach($categories as $key=>$cat_data)
                  <option value='{{$cat_data->id}}' {{(($product->cat_id==$cat_data->id)? 'sélectionné' : '')}}>{{$cat_data->title}}</option>
              @endforeach
          </select>
        </div>
        @php 
        ...
        <div class="form-group {{(($product->child_cat_id)? '' : 'd-none')}}" id="child_cat_div">
          ...
          <select name="child_cat_id" id="child_cat_id" class="form-control">
              <option value="">--Sélectionnez n'importe quelle sous-catégorie--</option>
        ...
        <div class="form-group">
          ...
          <select name="size[]" class="form-control selectpicker"  multiple data-live-search="true">
              <option value="">--Sélectionnez n'importe quelle taille--</option>
        ...
        <div class="form-group">
          ...
          <select name="brand_id" class="form-control">
             ...
             @foreach($brands as $brand)
              <option value="{{$brand->id}}" {{(($product->brand_id==$brand->id)? 'sélectionné':'')}}>{{$brand->title}}</option>
             @endforeach
          </select>
        </div>
        ...
        <div class="form-group">
          ...
          <select name="condition" class="form-control">
              ...
              <option value="default" {{(($product->condition=='default')? 'sélectionné':'')}}>Par défaut</option>
              <option value="new" {{(($product->condition=='new')? 'sélectionné':'')}}>Nouveau</option>
              <option value="hot" {{(($product->condition=='hot')? 'sélectionné':'')}}>Top</option>
          </select>
        </div>
...
<script>
    ...
    $(document).ready(function() {
      $('#description').summernote({
        placeholder: "Écrire la description détaillée.....",
          tabsize: 2,
          height: 150
      });
    });
                        ...
                        if(response.status){
                            ...
                            if(response.data){
                                ...
                                $.each(data,function(id,title){
                                    html_option += "<option value='"+id+"' "+(child_cat_id==id ? 'sélectionné ' : '')+">"+title+"</option>";
                                });
                            }
                            else{
                                console.log('données sans réponse');
                            }
                        }
                        else{
